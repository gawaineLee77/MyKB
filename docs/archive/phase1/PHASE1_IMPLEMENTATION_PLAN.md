...
> Historical record: dates, task states, acceptance evidence, and commands below describe the recorded phase, not the current deployment. See [current status](../../CURRENT_STATUS.md), [open work](../../ROADMAP.md), and the [deployment runbook](../../guides/FULL_DEPLOYMENT_RUNBOOK_ZH.md). Archiving does not close unfinished tasks or waive release approval.
...
| Delivery rule | Implement and verify small tasks sequentially; continue within the authorized scope |
...
- Complete each task and its applicable acceptance checks before starting dependent work, then continue automatically within the user-authorized task range, Gate, or Phase. Follow [AGENTS.md](../../../AGENTS.md#autonomy--task-completion) for actual approval gates and partial blockers; a progress update does not require renewed approval.
...
- [x] **P1-01 — Route and policy inventory.** Record upstream route families as pass-through, product-owned, disabled, or KB-policy-controlled. **Accept:** every v0.7.2 route is classified and the Personal Notes enforcement points are listed. See [the verified route-policy inventory](../../reference/PHASE1_ROUTE_POLICY.md).
- [x] **P1-02 — Gateway skeleton.** Add a standalone service with configuration, structured errors, `/health`, and `/version`; no business behavior. **Accept:** unit tests and a local binary/container health check pass. See [the Phase 1 gateway guide](../../reference/PHASE1_GATEWAY.md).
...
At the initial Phase 1 kickoff, **P1-01** was reviewed before **P1-02** became the first code change. Both are now complete. That initial review checkpoint does not require restarting these tasks or pausing between tasks in a subsequently authorized implementation range. Preserve the recorded status of unfinished tasks until their acceptance criteria are met.

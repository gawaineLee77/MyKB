...
The Stage 2 RAG workspace supports Markdown, text, PDF, Word, spreadsheet, presentation, CSV, HTML, JSON, and XML files. MindCreek currently permits up to 500 MiB per file; operators can set `MAX_FILE_SIZE_MB` to a value from 1 through 500 at deployment time. The shared value configures the browser, edge Nginx, MindCreek Gateway, WeKnora App, and Docreader. The workspace displays queue/processing/final states, failure details, retry, cancel, native advanced view, and normal chat.

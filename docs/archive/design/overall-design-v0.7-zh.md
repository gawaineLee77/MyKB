> 历史设计 v0.7：2026-09-15 由空间改版设计取代。保留原内容与当时状态供追溯，不作为新产品需求。
...
| 当前批准的上游基线 | WeKnora v0.8.0 |
| 部署模式 | 面向内部用户的私有化部署服务 |
| 主要界面 | Web 应用 |

语言：中文 | [English version](overall-design-v0.7.md)

本文保留长期架构和交付历史，不只描述已上线功能。请结合[文档导航](../../README.md)、[当前能力](../../CURRENT_STATUS.md)和[后续待办](../../ROADMAP.md)，区分已实现能力、未来范围与目标服务器验收。
...
系统将围绕 [Tencent WeKnora](https://github.com/Tencent/WeKnora) 构建“上游优先”的发行版，当前固定在已批准的 [v0.8.0](https://github.com/Tencent/WeKnora/releases/tag/v0.8.0)。WeKnora 提供文档处理、检索、Wiki、智能体、工作空间和 RBAC 等基础能力。发布、订阅、组织公开访问、权限策略和产品 UI 行为放在隔离的扩展模块中。微信小程序、IM 频道、CLI、技能沙箱和智能体长期记忆等非目标功能仍保留在上游源码中，但不会被产品构建、启动、路由或展示。
...
| 文档 RAG | 多格式文档、URL、在线条目、获准连接器，或面向个人与共享知识的上游原生 FAQ 问答知识库。 | 用户选择主检索配置；`plain` 使用文本分块与混合检索，FAQ 使用结构化问答分块，`graph` 增加图索引，`pixel` 增加页面/图块视觉索引。 | MVP 支持 Plain 与 FAQ；图和像素后续实现 |
...
FAQ 问答是受治理 RAG 产品模式下的上游知识库类型，而不是第四种产品模式。创建界面沿用 WeKnora 原生名称（`问答` / `FAQ Q&A`），创建 `type=faq`，随后进入原生 FAQ 条目管理页完成表格导入、推荐标记和答案维护。
...
创建向导先选择产品模式/内容类型，再只显示相关配置。MVP 开放个人笔记、文档 Plain RAG 与上游原生 FAQ 问答；GraphRAG、PixelRAG 和本体在通过验收门禁前保持能力开关关闭或标记为预览。
...
5. 后台处理显示进度、失败原因、重试、取消、文档删除以及预计/实际模型成本；删除通过上游生命周期清理源文件及其全部派生分块和索引。
6. 未来启用 GraphRAG 或 PixelRAG 时，由适配器增加派生索引，并保留 Plain RAG 作为证据和回退路径。

对于 FAQ 问答，同一条受治理 RAG 创建链路会传递 `type=faq`，采用 WeKnora 原生的 `question_only` 与 `separate` 默认配置，然后交由原生 FAQ 页面管理。FAQ 表格可使用 JSON、CSV、XLSX 或 XLS 导入；已启用且允许推荐的标准问题可进入智能体开场问题推荐。
...
- 需要导入扫描版或纯图片 PDF 的部署可以额外配置稳定的可选默认模型 `builtin-mindcreek-vlm`。该模型健康时，新建文档 RAG 会自动启用 WeKnora 已有的页面 OCR/描述流程；个人笔记仍保持纯文本，原有三模型就绪条件保持向后兼容。
...
- 就绪探针在不暴露密钥的前提下验证三类必需托管模型以及已经配置的可选 VLM。轮换时保持稳定模型 ID，先在预发布环境验证替代凭据；除非执行明确的凭据重加密流程，否则必须保留现有 `SYSTEM_AES_KEY`。

## 8. 逻辑架构

![内部知识库智能体平台架构](../../assets/internal-kb-architecture-v0.4.png)
...
- 生产环境固定到经过批准的发布标签，当前为 [v0.8.0](https://github.com/Tencent/WeKnora/releases/tag/v0.8.0)；不得部署未固定的 `main` 构建。
...
以下阶段 0–5 保留原交付范围；阶段计划及证据见[历史归档](../README.md)。Phase 1 未关闭项和 Phase 5 运维条件仍在[当前待办](../../ROADMAP.md)中跟踪。部署当前服务无需重新执行已完成的开发阶段。

### 阶段 0 — 基线与清单

**结果：** 可复现的原始 v0.8.0 部署，以及稳定集成边界清单。
...
**结果：** 保留原始知识库和智能体功能，优先提供个人笔记、Plain RAG 与原生 FAQ 问答，同时不交付或暴露小程序、CLI 和 IM 功能。
...
- 简化导航和品牌界面。
- 增加模式驱动的创建向导，开放个人笔记、文档 RAG/Plain 与 WeKnora 原生 FAQ 问答。
...
- 仅允许明确配置为不使用知识库范围的已认证智能体进行无 RAG 的纯对话；此类请求不获得任何文档访问权限，也不产生知识来源引用。
...
- 创建向导提供个人笔记、文档 RAG/Plain 与原生 FAQ 问答；GraphRAG、PixelRAG 和本体保持禁用或明确标记为实验。
...
**决策：** 使用最新批准的 WeKnora 正式标签——当前为 v0.8.0——作为位于产品扩展边界之后的上游核心。
...
**决策：** 复用 WeKnora 的声明式内置模型机制，为部署提供一组由管理员管理的默认对话、Embedding 和 Rerank 模型，并为扫描件 Plain RAG 提供可选的视觉/OCR 模型。密钥仅在运行时注入。普通用户自动使用默认模型，看不到凭据或敏感端点；用户自定义供应商属于默认关闭的“高级设置”能力，并实施私有所有权和独立策略控制。

**原因：** 知识产品不应要求每位用户都理解模型供应商或持有组织凭据。复用上游内置模型可降低升级成本；运行时密钥注入、响应脱敏和严格限制的自定义模型可避免便利性演变为凭据或数据出站风险。

## 22. 实施前待定事项

下表保留设计默认值及后续选择，不要求重新启动阶段 0。已落地行为以当前实现和部署指南为准；未完成开发及环境验收见[当前待办](../../ROADMAP.md)。表中的建议值不代表某项功能或越权机制已经启用。
...
- [WeKnora v0.8.0 发布版本](https://github.com/Tencent/WeKnora/releases/tag/v0.8.0)
...
从[当前待办](../../ROADMAP.md)选择下一项已获授权的工作。部署时遵循[完整执行手册](../../guides/FULL_DEPLOYMENT_RUNBOOK_ZH.md)和[受控试点要求](../../guides/PHASE5_PILOT.md)，复用仍有效的环境证据并保留必要批准。阶段 6 GraphRAG 实验必须与稳定 Plain RAG 路径隔离，并且只能在基准契约获批后启动。

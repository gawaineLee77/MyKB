> Historical v0.7 design: superseded by the workspace redesign on 2026-09-15. Original content and status are retained as historical evidence, not current requirements.
...
| Current approved upstream baseline | WeKnora v0.8.0 |
| Deployment model | Self-hosted service for internal users |
| Primary interface | Web application |

Language: English | [中文版](overall-design-v0.7-zh.md)

This document retains the long-term architecture and delivery history, not just shipped features. Use the [documentation index](../../README.md), [current capability summary](../../CURRENT_STATUS.md), and [open-work roadmap](../../ROADMAP.md) to distinguish implementation, future scope, and target-server acceptance.
...
| Document RAG | Multi-format documents, URLs, entries, approved connectors, or an upstream-native FAQ Q&A knowledge base for individual or shared knowledge. | User selects a primary profile. `plain` uses text chunks and hybrid retrieval; FAQ uses structured question/answer chunks; `graph` adds graph indexes; `pixel` adds page/tile visual indexes. | Plain and FAQ in MVP; graph and pixel later |
...
FAQ Q&A is an upstream knowledge-base type inside the governed RAG product mode, not a fourth product mode. The creation UI uses WeKnora's native labels (`FAQ Q&A` / `问答`), creates `type=faq`, and opens the native FAQ entry manager for spreadsheet import, recommendation flags, and answer maintenance.
...
The creation wizard first asks for product mode/content type, then shows only relevant configuration. In the MVP, Personal Notes, Document RAG with the Plain profile, and upstream-native FAQ Q&A are enabled; GraphRAG, PixelRAG, and Ontology remain capability-gated previews until their acceptance gates pass.
...
For FAQ Q&A, the same governed RAG creation path sends `type=faq` with WeKnora's native `question_only` and `separate` defaults, then hands management to the native FAQ page. FAQ tables may be imported as JSON, CSV, XLSX, or XLS; recommended, enabled standard questions can feed agent starter suggestions.
...
![Internal Knowledge Base Intelligent Agent Platform architecture](../../assets/internal-kb-architecture-v0.4.png)
...
- Pin production to an approved release tag, currently [v0.8.0](https://github.com/Tencent/WeKnora/releases/tag/v0.8.0); never deploy an unpinned `main` build.
...
- Adapt the current corporate plain OAuth 2.0 profile with a CSP-restricted browser form POST to `/authorize` containing `response_type=code` and `display=page`, a server-side JSON POST to `/accesstoken`, and a server-side GET to `/userinfo`. Register the MindCreek external origin as the corporate redirect URI and forward its root `?code=...` callback to the gateway before the SPA starts. Because the provider does not currently return state or accept PKCE in the described exchange, bind the transaction to a Secure, HttpOnly, SameSite callback cookie; retain configurable state and PKCE and request provider support as the preferred hardening path.
...
Phases 0–5 below retain their delivery scope. Historical plans and evidence are in the [archive](../README.md); unresolved Phase 1 items and Phase 5 operator conditions remain visible in the [active roadmap](../../ROADMAP.md). Do not replay completed phases to deploy the current service.

### Phase 0 — Baseline and inventory

**Outcome:** A reproducible, unmodified v0.8.0 deployment and a map of stable integration seams.
...
**Outcome:** Stock knowledge and agent functionality plus the priority Personal Notes, Plain RAG, and native FAQ Q&A paths, without shipping or exposing Mini Program, CLI, or IM features.
...
- Add the mode-driven creation wizard with Personal Notes, Document RAG/Plain, and WeKnora-native FAQ Q&A enabled.
...
- The creation wizard exposes Personal Notes, Document RAG with Plain RAG, and native FAQ Q&A; GraphRAG, PixelRAG, and Ontology remain disabled or clearly experimental.
...
**Decision:** Use the latest approved tagged WeKnora release—currently v0.8.0—as an upstream-owned core behind product extension boundaries.
...
The table retains design defaults and deferred choices; it is not a request to restart Phase 0. Consult the current implementation and deployment guides for settled behavior, and the [active roadmap](../../ROADMAP.md) for unfinished development and environment-specific acceptance. A recommended default is not evidence that a feature or override is enabled.
...
- [WeKnora v0.8.0 release](https://github.com/Tencent/WeKnora/releases/tag/v0.8.0)
...
Select the next authorized task from the [active roadmap](../../ROADMAP.md). For deployment, follow the [current runbook](../../guides/FULL_DEPLOYMENT_RUNBOOK_ZH.md) and [controlled-pilot requirements](../../guides/PHASE5_PILOT.md), reusing valid environment evidence while retaining required approval. Keep Phase 6 GraphRAG isolated from the stable Plain RAG path and begin only with an approved benchmark contract.

...
The maintained Web Ask/MCP instructions are now in the [agent and MCP guide](../../guides/AGENT_AND_MCP.md). Build and rollback commands below remain historical references only.

## Release contents

MindCreek `0.5.0-phase4` keeps the approved WeKnora v0.8.0 baseline unmodified and adds the authorized Web Ask workspace and hosted read-only MCP facade. The runtime uses seven images listed in `images/manifests/phase4-runtime.txt`; no Neo4j, SearXNG, upstream MCP server, CLI, IM, Mini Program, skill-sandbox, or long-term-memory service is required.

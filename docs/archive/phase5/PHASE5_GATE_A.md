...
Normal creation injects managed embedding and chat IDs server-side and records the managed reranker in the immutable product profile. Quick Answer and Smart Reasoning are explicitly registered against the managed chat/rerank pair with web search disabled. The chat surface repairs stale browser selections after the model list loads and prefers `builtin-mindcreek-chat` when no valid user choice exists.

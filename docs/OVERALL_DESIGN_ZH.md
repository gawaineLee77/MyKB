...
**实施状态（2026-08-30）：** 阶段 5 门禁 A 已完成，包含稳定托管模型 ID、无密钥部署渲染器、脱敏模型门面、自动默认选择，以及默认关闭的“高级设置”覆盖模型生命周期。生产供应商启用仍由运维人员通过密钥管理流程完成。
...
- 使用带 PKCE 的 OAuth 2.0 Authorization Code Flow，并从身份提供商经过验证的 Token 或用户信息端点获取稳定的组织用户标识。
- 关闭公开注册和不受组织限制的邀请链接。
- 使用 Secure、HTTP-only、SameSite Cookie，或同等安全的 Bearer Token 管理方式。
- 密码重置、用户停用或重要角色变化后撤销会话。
- 管理员按企业策略使用更强认证方式。
...
- 集成组织的 OAuth 2.0 身份提供商并关闭自助注册。
- 完成网络隔离、TLS、密钥、备份、恢复、日志、指标和告警。
- 执行安全、压力、迁移和恢复测试。
- 选择少量团队试点并收集可用性和检索质量反馈。
- 审查下游补丁清单，删除已经被上游替代的补丁；不得删除被排除的上游模块。
...
从[阶段 5 Gate A](PHASE5_IMPLEMENTATION_PLAN.md) 的托管模型契约和无密钥部署模板开始。将真实值放入受保护的本地密钥源之前，先确认供应商协议、模型名称和 Embedding 维度。用户模型自定义只有在所有权、出站、加密和审计矩阵通过后才能启用。

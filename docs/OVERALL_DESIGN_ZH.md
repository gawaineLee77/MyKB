...
| 模型部署 | 条件允许时使用内部 Chat/Embedding/Rerank 端点 |
...
将未修改的 WeKnora v0.7.2 源码导入或固定为边界明确的上游部分。阶段 0 随后验证原始构建和部署，清点公开 API 与扩展点，并为个人笔记、Plain RAG 和可选 LLM Wiki 建立小型概念验证映射。开始产品开发前，应强制执行[下游补丁台账](UPSTREAM_PATCHES.md)、版本化适配器契约测试和自动上游边界检查。

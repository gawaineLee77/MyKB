...
The Stage 2 RAG workspace supports Markdown, text, PDF, Word, spreadsheet, presentation, CSV, HTML, JSON, and XML files. MindCreek permits up to 200 MiB per file; operators can set `MAX_FILE_SIZE_MB` to a value from 1 through 200 at deployment time. The shared value configures the browser, edge Nginx, MindCreek Gateway, WeKnora App, and Docreader. The workspace displays queue/processing/final states, failure details, retry, cancel, native advanced view, and normal chat.

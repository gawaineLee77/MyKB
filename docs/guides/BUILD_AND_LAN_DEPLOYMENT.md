...
| `wechatopenai/weknora-app:v0.8.0` | WeKnora Go application | Yes |
| `wechatopenai/weknora-docreader:v0.8.0` | PDF/Office/image parsing | Yes |
...
Inspect services with `make phase5-ps` and logs with `./scripts/phase5-compose.sh logs -f gateway app frontend`. Stop containers with `make phase5-down`; named volumes are preserved. Never use `docker compose down -v` unless permanent data deletion is intended. Back up PostgreSQL and `data-files` before upgrades, follow the [current deployment runbook](FULL_DEPLOYMENT_RUNBOOK_ZH.md) and [v0.8.0 upgrade record](../upgrades/WEKNORA_V0.8.0_UPGRADE.md), and promote a newly pinned WeKnora release only after the compatibility suite and required approval. Historical Gate A evidence is in [PHASE5_GATE_A.md](../archive/phase5/PHASE5_GATE_A.md).

...
> Maintained implementation reference. See [current status](../CURRENT_STATUS.md); phase names identify the original delivery, while current configuration and contract tests govern behavior.

| Field | Value |
|---|---|
| Task | P1-01 |
| Status | Complete |
| Upstream | WeKnora v0.8.0 at `1edcd54b43606d9079bb36650efe3f68707a79ea` |
| Standard and conditional routes | 429 |
| Machine policy | `config/phase1-route-policy.json` |

## Classification result

The inventory is built from the actual Gin registration functions, not Swagger annotations. It includes 408 standard routes plus 21 conditional public embed and resource-grant routes.

| Classification | Count | Phase 1 behavior |
|---|---:|---|
| `disabled` | 149 | Gateway returns `feature.disabled`; the service or credentials are also omitted. |
| `kb_policy_controlled` | 170 | Gateway resolves the authenticated principal and every direct or indirect KB reference before forwarding. |
| `pass_through` | 110 | Gateway preserves upstream authentication and RBAC, with request limits and audit metadata. |
...
| Approved `/initialization/**`, `/chunker/preview`, exact `/skills` | Pass-through | Required for Plain RAG configuration and retained agents; excluded subpaths match earlier deny rules. |
...
| Session artifacts and message-scoped files | KB-policy-controlled | Session ownership and persisted agent/KB scope are resolved before generated files or shared-KB images are served. |
| `/r/:token`, `/api/v1/files/presigned*` | Disabled | Capability and presigned URLs can bypass a fresh user/KB owner check. |
| Skill catalog writes, sandbox configs, personal sandbox environment variables, memory, and sandbox checks | Disabled | Skill execution and upstream long-term memory are outside the MindCreek Phase 5 product boundary. |
| `/api/v1/system/admin/users/create` | Disabled | Corporate OAuth remains the normal provisioning authority; local emergency-account handling stays separately controlled. |

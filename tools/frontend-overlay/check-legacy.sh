...
UPSTREAM_FRONTEND="$UPSTREAM_ROOT/frontend"
...
copy_anchor src/stores/editorResources.ts
copy_anchor src/utils/request.ts
copy_anchor src/api/auth/index.ts
copy_anchor src/App.vue
copy_anchor src/views/settings/ModelSettings.vue
copy_anchor src/views/settings/Settings.vue
...
node "$SCRIPT_DIR/apply-legacy.mjs" "$TARGET" "$REPO_ROOT/branding/mindcreek"
...
grep -q 'Promise.allSettled' "$TARGET/src/views/agent/AgentEditorModal.vue"
grep -q 'MindCreek excludes upstream Web Search, MCP services, and skill sandboxes' "$TARGET/src/views/agent/AgentEditorModal.vue"
grep -q 'const mindCreekExternalAgentToolsEnabled = false' "$TARGET/src/views/agent/AgentEditorModal.vue"
grep -q 'if (mindCreekExternalAgentToolsEnabled)' "$TARGET/src/views/agent/AgentEditorModal.vue"
grep -q 'MindCreek excludes upstream MCP-service and skill-sandbox configuration' "$TARGET/src/stores/editorResources.ts"
grep -q 'MINDCREEK_HIDDEN_SETTINGS_SECTIONS' "$TARGET/src/views/settings/Settings.vue"
grep -q "'memory'" "$TARGET/src/views/settings/Settings.vue"
grep -q "'sandbox'" "$TARGET/src/views/settings/Settings.vue"
grep -q "'integration-im'" "$TARGET/src/views/settings/Settings.vue"
grep -q 'const canManageSkills = computed(() => false)' "$TARGET/src/components/UserMenu.vue"
grep -q 'chooseChatModel' "$TARGET/src/components/Input-field.vue"
grep -q 'currentExists' "$TARGET/src/components/Input-field.vue"
grep -q 'settingsStore.selectKnowledgeBases(\[kbId\])' "$TARGET/src/components/Input-field.vue"
...
grep -q "t('knowledgeEditor.basic.typeFAQ')" "$TARGET/src/mindcreek/CreateKnowledgeSpace.vue"
grep -q "t('knowledgeEditor.basic.typeFAQ')" "$TARGET/src/mindcreek/KnowledgeLibrary.vue"
grep -q "knowledge_base_type: draft.mode === 'faq' ? 'faq' : 'document'" "$TARGET/src/mindcreek/contracts.ts"
grep -q "item.type === 'faq'" "$TARGET/src/mindcreek/KnowledgeLibrary.vue"
...
grep -q "path: '/platform/mindcreek/ask'" "$TARGET/src/mindcreek/RAGWorkspace.vue"
...
grep -q 'mergeDocumentReferences' "$TARGET/src/mindcreek/AskWorkspace.vue"
grep -q 'route.query.kb_id' "$TARGET/src/mindcreek/AskWorkspace.vue"
...
if [ "$UPSTREAM_ROOT" = "$REPO_ROOT/upstream/weknora" ]; then
  DIRTY=$(git -C "$UPSTREAM_ROOT" status --porcelain --untracked-files=all)
  test -z "$DIRTY" || {
...
echo "MindCreek Phase 5 overlay verified against $UPSTREAM_ROOT"

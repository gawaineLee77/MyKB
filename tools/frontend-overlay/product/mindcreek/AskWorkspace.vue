<template>
  <main class="mc-ask">
    <header class="ask-header">
      <button class="back" type="button" @click="router.push('/platform/knowledge-bases')"><t-icon name="chevron-left" /> {{ text.library }}</button>
      <div><span>MindCreek · {{ text.eyebrow }}</span><h1>{{ text.title }}</h1><p>{{ text.subtitle }}</p></div>
    </header>

    <div class="ask-layout">
      <aside class="scope-panel">
        <header><div><strong>{{ text.scope }}</strong><small>{{ selected.length }} / 64</small></div><p>{{ text.scopeHint }}</p></header>
        <div v-if="loading" class="state"><t-loading /> {{ text.loading }}</div>
        <div v-else-if="loadError" class="state error">{{ loadError }} <button type="button" @click="loadScopes">{{ text.retry }}</button></div>
        <template v-else>
          <section v-if="defaultItems.length"><h2>{{ text.defaultKnowledge }}</h2><label v-for="item in defaultItems" :key="item.id"><input type="checkbox" :checked="selected.includes(item.id)" @change="toggle(item.id)" /><span><b>{{ item.name }}</b><small>{{ sourceLabel(item.access_source) }}</small></span></label></section>
          <section v-if="publicItems.length"><h2>{{ text.publicKnowledge }}</h2><label v-for="item in publicItems" :key="item.id"><input type="checkbox" :checked="selected.includes(item.id)" @change="toggle(item.id)" /><span><b>{{ item.name }}</b><small>{{ text.explicitPublic }}</small></span></label></section>
          <p v-if="!defaultItems.length && !publicItems.length" class="state">{{ text.empty }}</p>
        </template>
        <footer><button type="button" @click="selectDefault">{{ text.selectDefault }}</button><button type="button" @click="selected = []">{{ text.clear }}</button></footer>
      </aside>

      <section class="conversation">
        <div class="mode-row"><button type="button" :class="{ active: mode === 'quick' }" @click="mode = 'quick'"><t-icon name="flash" /> {{ text.quick }}</button><button type="button" :class="{ active: mode === 'reasoning' }" :disabled="!reasoningModelId" :title="!reasoningModelId ? text.reasoningNeedsModel : ''" @click="mode = 'reasoning'"><t-icon name="chat-bubble-help" /> {{ text.reasoning }}</button><span><t-icon name="shield" /> {{ text.liveAuth }}</span></div>
...
<style scoped>
.mc-ask{min-height:100%;overflow:auto;padding:34px clamp(18px,4vw,58px) 48px;color:#173c35;background:radial-gradient(circle at 90% 0,#dff3eb 0,transparent 30%),#f7faf8}.ask-header{max-width:1240px;margin:0 auto 24px;display:flex;gap:20px;align-items:flex-start}.ask-header .back{margin-top:5px}.ask-header span{color:#1c8063;font-size:11px;font-weight:750;letter-spacing:1.6px;text-transform:uppercase}.ask-header h1{margin:4px 0 6px;font:650 32px/1.15 Georgia,serif}.ask-header p{margin:0;color:#6c827a}.mc-ask button{padding:8px 11px;border:1px solid #cadbd4;border-radius:9px;color:#315e51;background:#fff;cursor:pointer}.ask-layout{max-width:1240px;min-height:650px;margin:auto;display:grid;grid-template-columns:330px 1fr;border:1px solid #d9e6e1;border-radius:18px;overflow:hidden;background:#fff;box-shadow:0 18px 50px rgba(43,84,72,.08)}.scope-panel{display:flex;flex-direction:column;border-right:1px solid #e0ebe6;background:#f3f8f5}.scope-panel>header,.scope-panel>footer{padding:18px;border-bottom:1px solid #dfeae5}.scope-panel>header div{display:flex;justify-content:space-between}.scope-panel>header p{margin:7px 0 0;color:#70847c;font-size:12px;line-height:1.45}.scope-panel section{padding:15px 14px 0}.scope-panel h2{margin:0 5px 8px;color:#657b73;font-size:10px;letter-spacing:1px;text-transform:uppercase}.scope-panel label{padding:10px;display:flex;gap:10px;align-items:flex-start;border-radius:9px}.scope-panel label:hover{background:#e7f2ed}.scope-panel label span{display:flex;flex-direction:column;gap:3px}.scope-panel label b{font-size:13px}.scope-panel label small{color:#798c84}.scope-panel>footer{margin-top:auto;display:flex;gap:7px;border-top:1px solid #dfeae5;border-bottom:0}.state{padding:28px 18px;color:#71877e;text-align:center}.state.error,.stream-error{color:#a04444}.conversation{min-width:0;display:flex;flex-direction:column}.mode-row{padding:13px 18px;display:flex;gap:7px;align-items:center;border-bottom:1px solid #e3ece8}.mode-row button.active{color:#fff;border-color:#207d61;background:#207d61}.mode-row span{margin-left:auto;color:#56756b;font-size:11px}.answer{flex:1;padding:25px clamp(18px,4vw,48px);overflow:auto;background:linear-gradient(#fff,#fbfdfc)}.welcome{height:100%;display:grid;place-items:center;align-content:center;color:#73877f;text-align:center}.welcome>span{width:56px;height:56px;display:grid;place-items:center;border-radius:16px;color:#23775d;background:#dff1e9;font-size:26px}.welcome h2{margin:13px 0 5px;color:#315b4e}.welcome p{margin:0;max-width:470px}.question,.assistant{max-width:760px;margin:0 0 18px;padding:14px 17px;border-radius:14px}.question{margin-left:auto;background:#e4f2ec}.assistant{border:1px solid #dce9e4;background:#fff}.question small,.assistant small{color:#28745d;font-weight:700}.question p,.assistant p{margin:6px 0 0;white-space:pre-wrap;line-height:1.65}.assistant i{display:inline-block;width:7px;height:16px;margin-left:3px;background:#27785e;animation:blink 1s infinite}.references{max-width:760px;margin-top:22px}.references h3{font-size:12px;color:#647a72;text-transform:uppercase}.references button{width:100%;margin:5px 0;display:flex;gap:10px;text-align:left}.references button span{display:flex;flex-direction:column}.references small{color:#80918a}.composer{margin:0 22px 22px;padding:12px;border:1px solid #cbded6;border-radius:14px;box-shadow:0 8px 26px rgba(43,84,72,.08)}.composer textarea{width:100%;min-height:90px;resize:vertical;border:0;outline:0;font:14px/1.5 inherit}.composer footer{display:flex;justify-content:space-between;align-items:center;color:#71867e;font-size:11px}.composer footer button{color:#fff;border-color:#207d61;background:#207d61}.composer button:disabled{opacity:.5}.back{white-space:nowrap}@keyframes blink{50%{opacity:.2}}@media(max-width:800px){.ask-layout{grid-template-columns:1fr}.scope-panel{max-height:330px;overflow:auto;border-right:0;border-bottom:1px solid #dfeae5}.ask-header{flex-direction:column}.mc-ask{padding:20px 12px 30px}.conversation{min-height:620px}}
</style>

<template>
  <main class="mc-library">
    <header class="mc-library-header">
      ...
      <div class="header-actions"><button type="button" data-testid="ask-button" @click="router.push('/platform/mindcreek/ask')"><t-icon name="chat" /> {{ text.ask }}</button><button class="primary" type="button" @click="router.push('/platform/mindcreek/create')"><t-icon name="add" /> {{ text.create }}</button></div>
    </header>
    ...
    <section v-else class="mc-content" aria-live="polite">
      ...
      <div v-else class="mc-grid">
        <article v-for="item in page.items" :key="item.id" class="mc-card" @click="open(item)">
          <header>
            <span class="mode-icon" :class="item.type === 'faq' ? 'faq' : item.product_mode"><t-icon :name="item.product_mode === 'personal_notes' ? 'edit-1' : item.type === 'faq' ? 'chat-bubble-help' : 'layers'" /></span>
            <span class="role" :class="item.role">{{ roleLabel(item.role) }}</span>
            <b v-if="item.updated" class="updated">{{ text.updated }}</b>
          </header>
          <div><small>{{ modeLabel(item.product_mode, item.type) }}</small><h2>{{ item.name }}</h2><p>{{ item.description || text.noDescription }}</p></div>
          <footer>
...
const router = useRouter(), { locale, t } = useI18n()
...
function modeLabel(mode?: string, type?: string) { if (mode === 'personal_notes') return locale.value.startsWith('zh') ? '私人笔记' : 'Personal Notes'; if (type === 'faq') return t('knowledgeEditor.basic.typeFAQ'); return locale.value.startsWith('zh') ? '文档 RAG' : 'Document RAG' }

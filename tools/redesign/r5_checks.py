#!/usr/bin/env python3
"""Current R5 executable checks; only cached local/synthetic dependencies."""
import datetime
import hashlib
import json
import os
...
import subprocess
import time
from r5_evidence import ROOT, source_digest, ui_digest, visual_review_valid


def main():
    target=ROOT/'.local/redesign-r5'
    target.mkdir(parents=True,exist_ok=True)
    report={'time':datetime.datetime.now(datetime.timezone.utc).isoformat(), 'status':'running',
            'gateway_source_sha256':source_digest(), 'ui_source_sha256':ui_digest(), 'checks':[],
            'scope':'Synthetic executable regression; R5 native UI checks. Actual synthetic browser/API evidence is recorded separately; no production deployment acceptance.'}
    env={**os.environ,'GOCACHE':str(ROOT/'.local/gateway-go-build'),'GOPROXY':'off','GOSUMDB':'off'}
    node=os.environ.get('R5_NODE_BIN','node')
    version=subprocess.check_output([node,'--version'],text=True).strip()
    if not version.startswith('v24.'): raise RuntimeError('R5 frontend compatibility requires Node 24 via R5_NODE_BIN')
    ...
    try:
        ...
        run('gateway-race',['go','-C','services/gateway','test','-race','-count=1','./internal/assistant','./internal/nativeaccess','./internal/mcp','./internal/enterprise','./internal/identity','./internal/weknora','./internal/server'])
        run('phase0',['make','phase0-check'])
        run('phase5',['make','phase5-check'])
        run('stage1',['make','stage1-check'])
        build=json.loads((ROOT/'docs/plans/evidence/r5-ui-build.json').read_text())
        if build['status']!='passed' or build['ui_source_sha256']!=ui_digest(): raise RuntimeError('Current frontend build required')
        report['checks'].append({'name':'frontend-node24','command':['sh','scripts/r5-build-ui.sh'],'exit_code':0,'evidence':'r5-ui-build.json','log_sha256':hashlib.sha256((ROOT/'.local/r5-frontend-check.log').read_bytes()).hexdigest()})
        ...
        run('r5-compose',['scripts/r4-compose.sh','config','--quiet'],{'MINDCREEK_R4_ENV_FILE':str(envfile),'MINDCREEK_R4_SECRET_DIR':str(secrets)})
        run('r5-shell',['sh','-n','scripts/r5-build-ui.sh','scripts/r4-compose.sh','scripts/r2-install.sh'])
        if report['gateway_source_sha256']!=source_digest() or report['ui_source_sha256']!=ui_digest():
        ...
        report['screenshots_inspected']=visual_review_valid()
        report['status']='passed'
    except Exception as error:
        report['status']='failed';report['error']=str(error)
        raise
    finally:
        (ROOT/'docs/plans/evidence/r5-checks.json').write_text(json.dumps(report,indent=2)+'\n')
